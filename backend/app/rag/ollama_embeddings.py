"""Ollama embedding service for local/offline retrieval (nomic-embed-text)."""
...
class OllamaEmbeddingService:
    """Generate 768-dimensional embeddings with a local Ollama model.

    Mirrors ``JinaEmbeddingService`` but talks to a local Ollama server's
    ``/api/embeddings`` endpoint (one prompt per request). The returned vector
    must be 768-d to match the ``embedding_ollama`` column / ``vector(768)``.
    """

    _instance: ClassVar["OllamaEmbeddingService | None"] = None

    def __new__(cls, *args: object, **kwargs: object) -> "OllamaEmbeddingService":
    ...
    def _extract_embedding(self, data: dict[str, Any]) -> list[float]:
        ...
        if len(embedding) != self.dimensions:
            raise LLMInvalidRequestError(
                f"Ollama embedding has {len(embedding)} dimensions, " f"expected {self.dimensions}"
            )
        return embedding
